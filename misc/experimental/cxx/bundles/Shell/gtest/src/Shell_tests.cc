...
TEST_F(ShellTest, LbCommandTest) {
    ...
    size_t pos = output.find("0: Framework");
    EXPECT_LE(pos, output.size());
    pos = output.find("1: Shell");
    EXPECT_LE(pos, output.size());
...
TEST_F(ShellTest, HelpCommandTest) {
    std::string filter = std::string{"("} + celix::SHELL_COMMAND_FUNCTION_COMMAND_NAME + "=help)";
    {
        ...
        bool called = framework().context()->buildUseFunctionService<celix::ShellCommandFunction>(celix::SHELL_COMMAND_FUNCTION_SERVICE_NAME)
                .setCallback(useHelp)
                .setFilter(filter)
                .use();
        EXPECT_TRUE(called);
...

    {
        ...
        ss.clear();
        called = framework().context()->buildUseFunctionService<celix::ShellCommandFunction>(celix::SHELL_COMMAND_FUNCTION_SERVICE_NAME)
                .setCallback(useHelp)
                .setFilter(filter)
                .use();
        EXPECT_TRUE(called);
