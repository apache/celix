...
TEST_F(FilterTest, CreateInvalidFilterTest) {
    ...
    EXPECT_ANY_THROW(f = "no-parentheses");

    //empty with whitespaces is also an invalid filter.
    EXPECT_ANY_THROW(f = " ");

    EXPECT_ANY_THROW(f = "(attr=trailing_stuff) fdafa");

    //test missing opening brackets in main filter
    EXPECT_ANY_THROW(f = "&(test_attr1=attr1)(|(test_attr2=attr2)(test_attr3=attr3))");
    ...
    EXPECT_ANY_THROW(f = "(&(test_attr1=attr1)(!test_attr2=attr2)");
    ...
    //test missing closing brackets in substring
    EXPECT_ANY_THROW(f = "(&(test_attr1=attr1)(|(test_attr2=attr2)(test_attr3=at(tr3)))");

    //test missing closing brackets in value
    EXPECT_ANY_THROW(f = "(&(test_attr1=attr1)(|(test_attr2=attr2)(test_attr3>=att(r3)))");

    //test trailing chars
    EXPECT_ANY_THROW(f = "(&(test_attr1=attr1)(|(test_attr2=attr2)(test_attr3=attr3))) oh no! trailing chars");

    //test half APPROX operator (should be "~=", instead is "~")
    EXPECT_ANY_THROW(f = "(&(test_attr1=attr1)(|(test_attr2=attr2)(test_attr3~attr3)))");;

    //test parsing a attribute of 0 length
    EXPECT_ANY_THROW(f = "(>=attr3)");

    //test parsing a value of 0 length
    EXPECT_ANY_THROW(f = "(test_attr3>=)");
}
