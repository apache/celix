...
celix_status_t eventAdmin_createEvent(event_admin_pt event_admin, const char *topic, properties_pt properties,
	...
	logHelper_log(*event_admin->loghelper, CELIX_LOG_LEVEL_DEBUG, "create event event admin pointer: %p",event_admin);


	*event = calloc(1, sizeof(**event));
	if(!*event){
	       status = CELIX_ENOMEM;
	       logHelper_log(*event_admin->loghelper, CELIX_LOG_LEVEL_ERROR, "No MEM");
	}else {
		logHelper_log(*event_admin->loghelper, CELIX_LOG_LEVEL_INFO, "Event created : %s", topic);
		(*event)->topic = topic;
