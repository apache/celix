 ...
 * can be used to dynamically alloc, initialize, dealloc, read & write structure
...
/**
 * Allocates memory for a type instance described by a dyn type. The memory will
 ...
 * This this _not_ includes sizes of data where is only pointed to (i.e. content
...
CELIX_DFI_EXPORT int dynType_metaEntries(dyn_type* type,
                                         struct meta_properties_head** entries);
 ...
 *
 * e.g. for a struct { int a; int b; }, a will have an index of 0 and b an index
 * of 1.
...
CELIX_DFI_DEPRECATED_EXPORT dyn_type* dynType_parseAvpr(FILE* avprStream,
                                                        const char* fqn);
CELIX_DFI_DEPRECATED_EXPORT dyn_type* dynType_parseAvprWithStr(const char* avpr,
                                                               const char* fqn);
