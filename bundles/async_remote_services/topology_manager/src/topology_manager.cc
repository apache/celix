...
void celix::async_rsa::AsyncTopologyManager::setDiscovery(celix::async_rsa::IDiscovery *discovery, [[maybe_unused]] Properties &&properties) {
    std::unique_lock l(_m);
...
public:
    ...
    _cmp(mng->createComponent(std::make_unique<celix::async_rsa::AsyncTopologyManager>(celix_logHelper_create(mng->bundleContext(), "celix_async_rsa_admin")))) {
        ...
        _cmp.createServiceDependency<celix::async_rsa::IDiscovery>()
                .setRequired(true)
