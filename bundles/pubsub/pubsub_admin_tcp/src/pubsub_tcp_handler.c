...
static inline int pubsub_tcpHandler_closeConnectionEntry(
  ...
  if (handle != NULL && entry != NULL) {
    ...
    if (entry->fd >= 0) {
      if (handle->receiverDisconnectMessageCallback) handle->receiverDisconnectMessageCallback(handle->receiverConnectPayload, entry->url, lock);
      if (handle->acceptConnectMessageCallback) handle->acceptConnectMessageCallback(handle->acceptConnectPayload, entry->url);
      pubsub_tcpHandler_freeEntry(entry);
      entry = NULL;
    }
  }
...
int pubsub_tcpHandler_listen(pubsub_tcpHandler_t *handle, char *url) {
  ...
  if (entry == NULL) {
    ...
    if ((rc >= 0) && (handle->efd >= 0)) {
      ...
      if (entry) L_INFO("[TCP Socket] Using %s for service annunciation", entry->url);
      if (entry) hashMap_put(handle->interface_fd_map, (void *)(intptr_t)entry->fd, entry);
      if (entry) hashMap_put(handle->interface_url_map, entry->url, entry);
    }
    celixThreadRwlock_unlock(&handle->dbLock);
  }
  return rc;
...
int pubsub_tcpHandler_dataAvailable(pubsub_tcpHandler_t *handle, int fd, unsigned int *index, bool *readMsg) {
    ...
    *readMsg = false;
    ...
    // Read the message
    entry->msg.msg_iovlen = 0;
    entry->msg.msg_iov[entry->msg.msg_iovlen].iov_base = (entry->headerBufferSize) ? entry->headerBuffer : entry->buffer;
    entry->msg.msg_iov[entry->msg.msg_iovlen].iov_len  = entry->headerSize;
    ...
    if (nbytes > 0) {
        ...
        } else if (handle->protocol->decodeHeader(handle->protocol->handle, entry->msg.msg_iov[entry->msg.msg_iovlen].iov_base, entry->msg.msg_iov[entry->msg.msg_iovlen].iov_len, &entry->header) != CELIX_SUCCESS) {
            entry->msg.msg_iov[0].iov_len = entry->syncSize;
            nbytes = recvmsg(fd, &entry->msg,  0);
            if (nbytes > 0)  entry->retryCount = 0;
            celixThreadRwlock_unlock(&handle->dbLock);
            return nbytes;
        }
        ...
        if (entry->header.header.metadataSize > entry->metaBufferSize) {
            ...
            if (buffer) {
                ...
                L_WARN("[TCP Socket] socket: %d, url: %s,  realloc read meta buffer: (%d, %d) \n", entry->fd, entry->url, entry->metaBufferSize, entry->header.header.metadataSize);
            }
        }
        ...
        if(entry->retryCount < handle->maxRcvRetryCount) {
            entry->retryCount++;
            L_WARN("[TCP Socket] Failed to receive message header (fd: %d), error: %s. Retry count %u of %u,", entry->fd, strerror(errno), entry->retryCount, handle->maxRcvRetryCount);
        } else {
            L_ERROR("[TCP Socket] Failed to receive message header (fd: %d) after %u retries! Closing connection... Error: %s", entry->fd, handle->maxRcvRetryCount, strerror(errno));
    ...
    if (nbytes > 0) {
        ...
        if (nbytes == msgSize) {
            *readMsg = true;
            ...
            L_ERROR("[TCP Socket] Failed to receive complete message (fd: %d) nbytes : %d = msgSize %d", entry->fd, nbytes, msgSize);
        }
    }
    else {
        if(entry->retryCount < handle->maxRcvRetryCount) {
            entry->retryCount++;
            L_WARN("[TCP Socket] Failed to receive message (fd: %d), error: %s. Retry count %u of %u,", entry->fd, strerror(errno), entry->retryCount, handle->maxRcvRetryCount);
        } else {
            L_ERROR("[TCP Socket] Failed to receive message (fd: %d) after %u retries! Closing connection... Error: %s", entry->fd, handle->maxRcvRetryCount, strerror(errno));
            nbytes = 0; //Return 0 as indicator to close the connection
        }
    }
    celixThreadRwlock_unlock(&handle->dbLock);
...
int pubsub_tcpHandler_read(pubsub_tcpHandler_t *handle, int fd, unsigned int index __attribute__ ((__unused__)),
    ...
    celixThreadRwlock_readLock(&handle->dbLock);
    psa_tcp_connection_entry_t *entry = hashMap_get(handle->interface_fd_map, (void *)(intptr_t)fd);;
    if (entry == NULL) entry = hashMap_get(handle->connection_fd_map, (void *)(intptr_t)fd);
    if (entry == NULL) result = -1;
    if (entry) result = (!entry->connected) ? -1 : result;
    if (!result) {
        ...
        if (entry->header.header.metadataSize > 0) {
            handle->protocol->decodeMetadata(handle->protocol->handle, entry->metaBuffer, entry->header.header.metadataSize, &entry->header);
        }
        *header = &entry->header;
    }
    celixThreadRwlock_unlock(&handle->dbLock);
...
//
int pubsub_tcpHandler_write(pubsub_tcpHandler_t *handle, pubsub_protocol_message_t* message, struct iovec* msgIoVec, size_t msg_iov_len, int flags) {
    celixThreadRwlock_readLock(&handle->dbLock);
    int result = 0;
...
void pubsub_tcpHandler_acceptHandler( pubsub_tcpHandler_t *handle, psa_tcp_connection_entry_t *pendingConnectionEntry) {
    ...
    if (rc >= 0){
        ...
        if (rc < 0) {
            ...
            if (handle->acceptConnectMessageCallback) handle->acceptConnectMessageCallback(handle->acceptConnectPayload, url);
            hashMap_put(handle->connection_fd_map, (void *)(intptr_t)entry->fd, entry);
            hashMap_put(handle->connection_url_map, entry->url, entry);
...
void pubsub_tcpHandler_readHandler( pubsub_tcpHandler_t *handle, int fd) {
    ...
    if (rc <= 0) {
        // close connection.
        if (rc == 0) pubsub_tcpHandler_close(handle, fd);
        return;
    }
    if (readMsg) {
        // Handle data
        ...
        if (rc < 0) return;
        celixThreadRwlock_readLock(&handle->dbLock);
        if (handle->processMessageCallback && header != NULL && header->payload.payload != NULL && header->payload.length) {
...
void pubsub_tcpHandler_connectionHandler( pubsub_tcpHandler_t *handle, int fd) {
    ...
    if (entry)
        if ((!entry->connected)) {
            ...
            if (handle->receiverConnectMessageCallback) handle->receiverConnectMessageCallback(handle->receiverConnectPayload, entry->url, false);
            entry->connected = true;
            struct epoll_event event;
            bzero(&event, sizeof(event)); // zero the struct
            event.events = EPOLLIN | EPOLLRDHUP | EPOLLERR;
            ...
            if (rc < 0) L_ERROR("[TCP Socket] Cannot create epoll\n");
        }
    celixThreadRwlock_unlock(&handle->dbLock);
}
...
void pubsub_tcpHandler_handler(pubsub_tcpHandler_t *handle) {
    ...
    if (handle->efd >= 0) {
        ...
        for (int i = 0; i < nof_events; i++) {
            ...
            while (hashMapIterator_hasNext(&iter)) {
                psa_tcp_connection_entry_t *entry = hashMapIterator_nextValue(&iter);
                if (events[i].data.fd == entry->fd) pendingConnectionEntry = entry;
            }
