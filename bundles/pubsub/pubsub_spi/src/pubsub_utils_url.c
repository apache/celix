...
char* pubsub_utils_url_get_url(struct sockaddr_in *inp, char* protocol) {
  ...
  if ((getnameinfo((struct sockaddr*)inp ,sizeof(struct sockaddr_in), host, NI_MAXHOST, service, NI_MAXSERV, NI_NUMERICHOST) == 0) && (inp->sin_family == AF_INET)) { //NI_NUMERICSERV
...
void pubsub_utils_url_parse_url(char *_url, pubsub_utils_url_t* url_info) {
  ...
  if (interface) {
    *interface++ = '0';
    ...
    length = strlen(url);
    if (uri) {
      if ((length > 1) && isalpha(uri[1])) *uri = '\0';
    }
  }
  free(url_copy);
}
...
pubsub_utils_url_t* pubsub_utils_url_parse(char *url) {
  ...
  if (url_info->interface) {
    ...
    struct sockaddr_in* m_sin = pubsub_utils_url_getInAddr(url_info->interface, 0);
    char* interface_url = pubsub_utils_url_get_url(m_sin, NULL);
    free(m_sin);
    pubsub_utils_url_parse_url(interface_url , &interface_url_info);
