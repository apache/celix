...
TEST_F(WireProtocolV2Test, WireProtocolV2Test_EncodeHeader_Test) { // NOLINT(cert-err58-cpp)
    ...
    memcpy(exp+24, &pps, sizeof(uint32_t));
    uint32_t ppo = 0x02000000;
    memcpy(exp+28, &ppo, sizeof(uint32_t));
    uint32_t ils = 0x01000000;
    memcpy(exp+32, &ils, sizeof(uint32_t));

    ASSERT_EQ(status, CELIX_SUCCESS);
    ASSERT_EQ(36, headerLength);
    for (int i = 0; i < 36; i++) {
...
TEST_F(WireProtocolV2Test, WireProtocolV2Test_DecodeHeader_Test) { // NOLINT(cert-err58-cpp)
    ...
    celix_status_t status = pubsubProtocol_wire_v2_decodeHeader(nullptr, exp, 36, &message);
...
TEST_F(WireProtocolV2Test, WireProtocolV2Test_WireProtocolV2Test_DecodeHeader_IncorrectSync_Test) { // NOLINT(cert-err58-cpp)
    pubsub_protocol_wire_v2_t *wireprotocol;
    pubsubProtocol_wire_v2_create(&wireprotocol);
...
TEST_F(WireProtocolV2Test, WireProtocolV2Test_WireProtocolV2Test_DecodeHeader_IncorrectVersion_Test) { // NOLINT(cert-err58-cpp)
    pubsub_protocol_wire_v2_t *wireprotocol;
    pubsubProtocol_wire_v2_create(&wireprotocol);
