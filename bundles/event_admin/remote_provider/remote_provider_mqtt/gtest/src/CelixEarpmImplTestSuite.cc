...
public:
    ...
    void MqttClientStart() {
        for (int i = 0; i < 512; ++i) {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds {100});//Wait for the handler description expired
        }
...
public:
    ...
    void TestRemoteProvider(std::function<void (celix_event_admin_remote_provider_mqtt_t*)>testBody) {
        ...
        auto status = celix_earpm_addBrokerInfoService(earpm, &brokerInfo, props.get());
        EXPECT_EQ(status, CELIX_SUCCESS);

        testBody(earpm);

        status = celix_earpm_removeBrokerInfoService(earpm, &brokerInfo, props.get());
        EXPECT_EQ(status, CELIX_SUCCESS);
    ...
    void AddHandlerDescriptionAndCheck(celix_event_admin_remote_provider_mqtt_t* earpm, const char* description, const char* frameworkUUID = FAKE_FW_UUID) {
...
TEST_F(CelixEarpmImplTestSuite, AddBrokerInfoServiceTest) {
    auto earpm = celix_earpm_create(ctx.get());
...
TEST_F(CelixEarpmImplTestSuite, AddEventHandlerServiceTest) {
    ...
    eventHandlerService.handleEvent = [](void*, const char*, const celix_properties_t*) { return CELIX_SUCCESS; };
    std::unique_ptr<celix_properties_t, decltype(&celix_properties_destroy)> props{celix_properties_create(), celix_properties_destroy};
    celix_properties_setLong(props.get(), CELIX_FRAMEWORK_SERVICE_ID, 234);
    celix_properties_set(props.get(), CELIX_EVENT_TOPIC, "topic");
    auto status = celix_earpm_addEventHandlerService(earpm, &eventHandlerService, props.get());
    EXPECT_EQ(status, CELIX_SUCCESS);

    status = celix_earpm_removeEventHandlerService(earpm, &eventHandlerService, props.get());
    EXPECT_EQ(status, CELIX_SUCCESS);
